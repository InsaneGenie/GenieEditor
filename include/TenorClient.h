...
// One search result from Tenor.
struct TenorGif {
    QString id;
    QString description;   // Tenor's own alt text — used as the accessible/tooltip label
    QString previewUrl;    // small, low-frame-rate GIF for the results grid
    QString fullUrl;       // the GIF actually imported when chosen
    QSize fullSize;
};

// Thin wrapper over Tenor's v2 HTTP API.
//
// Tenor requires an API key (free, from Google Cloud). There is deliberately no
// key compiled in: a key embedded in a distributed binary is both a terms
// violation and trivially extractable, so it's asked for once and kept in
// QSettings.
//
...
// making a request.
class TenorClient : public QObject {
    Q_OBJECT
public:
    explicit TenorClient(QObject* parent = nullptr);
    ...
    // The search endpoint. Defaults to Tenor's, and can be redirected via the
    // TENOR_BASE_URL environment variable — which exists so the panel can be
    ...
    // Parses a Tenor v2 /search response. Returns an empty vector on malformed
    // input rather than throwing — a bad payload should show "no results", not
    // take the app down.
    static QVector<TenorGif> parseSearchResponse(const QByteArray& json);

signals:
    void searchFinished(const QVector<TenorGif>& results);
