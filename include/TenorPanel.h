...
// Searches Tenor and imports the chosen GIF into the project.
//
...
class TenorPanel : public QWidget {
    Q_OBJECT
public:
    explicit TenorPanel(QWidget* parent = nullptr);
    ~TenorPanel() override;
...
private:
    void runSearch();
    void showResults(const QVector<TenorGif>& results);
    ...
    QHash<QString, PreviewMovie> m_previews; // keyed by Tenor id
    QString m_hoveredId;
